...
async function syncToAirtableMirror(row) {
  if (!process.env.AIRTABLE_PAT || !process.env.AIRTABLE_BASE_ID) {
    await dal.airtable.updateStatusError('FAILED_RETRYABLE', 'AIRTABLE_NOT_CONFIGURED', row.entity, row.entity_id, row.central_version);
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (currentEntity && currentEntity.version === row.central_version) {
        await dal.airtable.updateStatus('COMPLETED', row.entity, row.entity_id, row.central_version);
      }
    } else {
      ...
      await dal.airtable.updateStatusError(newStatus, `HTTP ${res.status} ${errorBody}`, row.entity, row.entity_id, row.central_version);
    }
  } catch (err) {
      await dal.airtable.updateStatusError('FAILED_RETRYABLE', err.message, row.entity, row.entity_id, row.central_version);
  }
}
